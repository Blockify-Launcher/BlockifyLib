...
namespace BlockifyLib.Launcher.src
{
    public sealed class AssetChecker : IFileChecker
    {
        ...
        private DownloadFile[]? checkIndexAndAsset(MinecraftPath path, Version.Version version,
            IProgress<DownloadFileChangedEventArgs>? progress)
        {
        ...
        private void checkIndex(MinecraftPath path, Version.Version version)
        {
        ...
        public JObject? ReadIndex(MinecraftPath path, Version.Version version)
        {
            if (string.IsNullOrEmpty(version.AssetId))
                return null;
        ...
        public DownloadFile[]? CheckAssetFiles(MinecraftPath path, Version.Version version,
            IProgress<DownloadFileChangedEventArgs>? progress)
        {
            ...
            bool isVirtual = checkJsonTrue(index["virtual"]);
            bool mapResource = checkJsonTrue(index["map_to_resources"]);
            ...
            foreach (var item in list)
            {
                ...
                if (progressed % 50 == 0) // prevent ui freezing
                    progress?.Report(
                        new DownloadFileChangedEventArgs(TypeFile.Resource, this, "", total, progressed));
            }

            return downloadRequiredFiles.Distinct().ToArray(); // 10ms
        }

        private DownloadFile? checkAssetFile(string key, JToken job, MinecraftPath path, Version.Version version,
            bool isVirtual, bool mapResource)
        {
            ...
            if (!IOUtil.CheckFileValidation(hashPath, hash, CheckHash))
                return new DownloadFile(hashPath, AssetServer + hashName)
                {
